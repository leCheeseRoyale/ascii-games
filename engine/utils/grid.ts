...
export class GridMap<T> {
  ...
  private idx(col: number, row: number): number { return row * this.cols + col }
  ...
  find(fn: (col: number, row: number, value: T | null) => boolean): { col: number; row: number; value: T | null } | null {
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
...
export function gridToWorld(col: number, row: number, cellSize: number, offset: Vec2 = { x: 0, y: 0 }): Vec2 {
  return {
...
export function worldToGrid(x: number, y: number, cellSize: number, offset: Vec2 = { x: 0, y: 0 }): { col: number; row: number } {
  return {
